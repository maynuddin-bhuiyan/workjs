...
const loadProducts = () => {
  ...
  // GetDetails id declare
  const getDetails = document.getElementById('getDaelis');
  ...
  const showProducts = (products) => {
    ...
    for (const product of allProducts) {
      ...
      <img class="product-image" src=${image}></img>
        ...
        <h4>Counting: <span class="span">  ${product.rating.count}</span></h4>
        <h5>RateTing: <span class="span"> ${product.rating.rate} </span></h5>
        <h2>Price: $ ${product.price}</h2>
        <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-success">add to cart</button>
        <button onclick= getDetals(${product.id}) class="btn btn-danger">Details</button></div>
        `;
  ...
  //Deatils Function Call
  
  const getDetals =  name => {
    const url = `https://fakestoreapi.com/products/${name}`;
    fetch(url)
    .then(rep => rep.json())
    .then(data => inDetilsa(data))
  };
  
  
  // show all details in UI 
  const inDetilsa = details => {
    window.scrollTo(0, 40);
    getDetails.textContent = '';
      console.log(details)
      const div = document.createElement('div');
    ...
    <img style="width: 18rem; height: 200px;" class="card-img-top" src="${details.image}" alt="Card image cap">
    <div class="card-body style="">
      <h3 class="card-title">${details.title.slice(0, 20)}</h3>
      <h4 class="card-title">${details.category.slice(0, 20)}</h4>
      <h5 class="card-title">${details.description.slice(0, 50)}</h5>
      <span class="span"><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star-half-alt"></i></span>
  
      <h2 class="span">${details.rating.rate}</h2>
    </div>
  </div>
      `;
